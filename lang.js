...
(function () {
  ...
  function translateTexts(root) {
    ...
    while ((t = tw.nextNode())) {
      ...
      if (window.APANA_LANG === 'en') {
        var k = t.nodeValue.trim();
        if (k && EN[k] !== undefined) { t.__apanaFr = t.nodeValue; t.nodeValue = t.nodeValue.replace(k, EN[k]); }
      } else if (t.__apanaFr !== undefined) { t.nodeValue = t.__apanaFr; t.__apanaFr = undefined; }
    }
  ...
  function tick() {
    ...
    if (!list.length && !added && isHome && Date.now() - started > 300) { added = true; addSelector(); list = langSelects(document, []); }
    ...
    var now = Date.now();
    if (window.APANA_LANG === 'en' && now - lastText > 900) { lastText = now; translateTexts(document.body); }
  }
